...
function getTeacherGroups (options, callback) {
  ...
  buddyQuery(qry, function (err, res) {
    ...
    if (typeof res[0] !== 'undefined' && res[0].length > 0) {
      ...
      logger.info('[APPNAME: %s] [USERNAME: %s] [TYPE: %s] [SEARCH: %s] [MESSAGE: %s]', options.appName, options.username, options.type, options.search, 'No contact groups')
    ...
    logger.info('[APPNAME: %s] [USERNAME: %s] [TYPE: %s] [SEARCH: %s] [MESSAGE: %s]', options.appName, options.username, options.type, options.search, noGroupsMessage.error)
